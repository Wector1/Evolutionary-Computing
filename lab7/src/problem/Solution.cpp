...
Solution::Solution()
    : totalCost(std::numeric_limits<double>::max()), pathLength(0.0) {}

Solution::Solution(const std::vector<int> &selectedNodes,
...
void Solution::calculateCostAndLength(const TSPAInstance &instance) {
  ...
  for (size_t i = 0; i < selectedNodes.size(); ++i) {
    ...
    int to = selectedNodes[(i + 1) %
                           selectedNodes.size()]; // Wrap around to form a cycle
...
double Solution::getPathLength() const { return pathLength; }

const std::vector<int> &Solution::getSelectedNodes() const {
  return selectedNodes;
}

bool Solution::isEmpty() const { return selectedNodes.empty(); }
...
void Solution::addStep() { steps++; }
long long Solution::getSteps() const { return steps; }
void Solution::setSteps(long long step) { steps = step; }
