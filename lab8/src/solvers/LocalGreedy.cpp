...
Solution LocalGreedy::solve(const TSPAInstance& instance, int startingPoint) {
    ...
    while (improvement) {
        ...
        // Shuffle all moves for random evaluation order
        std::shuffle(allMoves.begin(), allMoves.end(), gen);
        
        // Evaluate moves in random order
        for (const auto& move : allMoves) {
            ...
            if (moveCategory == 0) {
                ...
                if (moveType == 0) {
                    ...
                    if (delta < 0) {
                        std::swap(selectedNodes[i], selectedNodes[j]);
                ...
                if (delta < 0) {
                    selectedNodes[nodeInIdx] = nodeOut;
                    currentSolution = Solution(selectedNodes, instance);
                    improvement = true;
                    break;
                }
