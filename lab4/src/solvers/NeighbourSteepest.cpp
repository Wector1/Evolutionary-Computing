...
Solution NeighbourSteepest::solve(const TSPAInstance &instance,
  ...
  std::vector<std::vector<std::pair<int, int>>> distances_and_costs((int)distanceMatrix.size(), std::vector<std::pair<int, int>>((int)distanceMatrix.size(), {0, 0}));
  ...
  while (improvement) {
    ...
    std::vector<int> nodesIndices(distanceMatrix.size());
      ...
      for (int i = 0; i < n; ++i) {
        ...
        for (int j = 1; j < 21; ++j) {
            if (unselectedNodesSet.count(distances_and_costs[i][j].second)) {
                ...
                if (node1 == node2)
                    continue;
                allMoves.push_back({1, 0, node1, node2}); // category=1 (Inter), type=0 (node)
                node1 = (nodesIndices[i] - 1 + n) % n;
                node2 = distances_and_costs[i][j].second;
                allMoves.push_back({1, 0, node1, node2}); // category=1 (Inter), type=0 (node)
            }
            else {
                int node1 = nodesIndices[i];
                int node2 = nodesIndices[distances_and_costs[i][j].second];
                allMoves.push_back({0, 1, std::min(node1, node2), std::max(node1, node2)}); // category=0 (Intra), type=1 (edge)
                node1 = (nodesIndices[i] - 1 + n) % n;
                node2 = (nodesIndices[distances_and_costs[i][j].second] - 1 + n) % n;
                allMoves.push_back({0, 1, std::min(node1, node2), std::max(node1, node2)}); // category=0 (Intra), type=1 (edge)
            }
        }
        // for (int j = i + 2; j < n; ++j) {
