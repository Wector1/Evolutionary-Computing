#include "NearestNeighbour2RegretWeighted.h"
...
Solution NearestNeighbour2RegretWeighted::solve(const TSPAInstance& instance, int startingPoint) {
    ...
    for (int i = 0; i < nodesToSelect - 1; i++) {
        ...
        int bestNode = -1;
        int maxWeightedScore = -INF;  // Weighted score instead of just regret
        int bestCost = INF;
        ...
        for (int j = 0; j < n; j++) {
            ...
            // Calculate weighted score: WEIGHT_REGRET * regret - WEIGHT_OBJECTIVE_FUNC_CHANGE * bestCost
            int weightedScore = WEIGHT_REGRET * regret - WEIGHT_OBJECTIVE_FUNC_CHANGE * costs[minIdx];
            
            // Select node with max weighted score
